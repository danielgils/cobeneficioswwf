...
#' @export
#'
# Here I merged run_ithim and ithim_calculation_sequence
run_model <- function(ithim_object, seed = 1) {
  ############################
  ## (0) SET UP
  set.seed(seed)
  # Put some global variables locally
  for (i in 1:length(ithim_object))
    assign(names(ithim_object)[i], ithim_object[[i]])

  # Dummy to get pathways health burden
  constant_mode <- TRUE
  ...
  RR_PA_AP_calculations <- combined_rr_ap_pa(ind_pa = RR_PA_calculations,
                                             ind_ap = RR_AP_calculations)
  ...
  injuries0 <- injuries_function_2(true_distances, injuries_list,
                                   reg_model, constant_mode)
  injuries_list <- NULL
  reg_model <- NULL
  true_distances <- NULL
  ...
  if (constant_mode) {
    pathway_hb <- join_hb_and_injury(pathway_hb_AP_PA,
                                     deaths_yll_injuries$deaths_yll_injuries)
    return(list( mmets = mmets_pp, scenario_pm = scenario_pm,
                 pm_conc_pp = pm_conc_pp, #co2_conc = co2_conc,
                injuries = injuries, ref_injuries = ref_injuries, hb = hb,
                pathway_hb = pathway_hb, whw = whw))
  }else{
