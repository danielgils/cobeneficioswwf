...
PA_dose_response <- function(cause, dose, confidence_intervals = F){
  ...
  outcome_type <- ifelse(cause %in% c('lung_cancer','breast_cancer',
                                      'endometrial_cancer','colon_cancer'),
                         'all' , 'mortality')

  # apply disease-specific thresholds
  if (cause %in% c('total_cancer','coronary_heart_disease','breast_cancer',
                   'endometrial_cancer','colon_cancer'))
    dose[dose > 35] <- 35
  ...
  if (PA_DOSE_RESPONSE_QUANTILE == T){
    rr <- qnorm(get(paste0('PA_DOSE_RESPONSE_QUANTILE_',cause)),
                mean = rr, sd = (ub - lb)/1.96)
    rr[rr < 0] <- 0
  }
