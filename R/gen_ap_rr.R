...
gen_ap_rr <- function(pm_conc_pp){
  ...
  pm_rr_pp$ap_age <- 0
  for (i in 1:length(min_ages)) # Categorize age for DR curves
    pm_rr_pp$ap_age[pm_rr_pp$age > min_ages[i]] <- min_ages[i] + 1

  pm_indices <- sapply(SCEN_SHORT_NAME, function(x)
  ...
  for (j in c(1:nrow(DISEASE_INVENTORY))[DISEASE_INVENTORY$air_pollution == 1]) {
    ...
    # Filter Dose-Response for specific cause
    dr_ap_disease <- DR_AP[DR_AP$cause_code == cause,]
    # apply by age groups
    ages <- unique(dr_ap_disease$age_code)
    for (age in ages) { # Loop for each age range in Dose-response
