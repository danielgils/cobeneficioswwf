...
#'
#' Creates any columns needed and missing from the trip set,
#' then writes the trip set to the global enviroment
#'
#'
#' @export
complete_trip_distance_duration <- function(){
  # Save trip_set in local environment
  trip_set <- TRIP_SET

  # A vector with the average speed for each trip according the mode used.
  stage_speed <- sapply(trip_set$stage_mode,
                        function(x){
  ...
  if (na_stage) {
    ...
    if ('stage_distance ' %in% colnames(trip_set) &&
       !is.na(sum(trip_set$stage_distance[!is.na(trip_set$stage_mode)]))) {
      cat('Populating stage duration from stage distance\n')
      trip_set$stage_duration <- trip_set$stage_distance / stage_speed * 60
    }else
      if ('trip_duration' %in% colnames(trip_set) &&
          !is.na(sum(trip_set$trip_duration[!is.na(trip_set$stage_mode)]))) {
        ## populate trip durations by summing stage durations
  ...
  if ('stage_duration' %in% colnames(trip_set) &&
      !'stage_distance' %in% colnames(trip_set))
  ...
  if (!'trip_distance' %in% colnames(trip_set)) {
    distances <- setDT(trip_set)[,sum(stage_distance), by = 'trip_id'] # Sum of stages
