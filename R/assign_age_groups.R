...
#'
#' @export
assign_age_groups <- function(dataset, age_category = AGE_CATEGORY,
                              age_lower_bounds = AGE_LOWER_BOUNDS,
                              max_age = MAX_AGE,
                              min_age = AGE_LOWER_BOUNDS[1], age_label = 'age'){
  # Filter dataset to age ranges considered and remove NAs
  dataset <- dataset[dataset[[age_label]] <= max_age &
                       !is.na(dataset[[age_label]]) &
                       dataset[[age_label]] >= min_age,]
  # Initialize column
  dataset$age_cat <- 0
